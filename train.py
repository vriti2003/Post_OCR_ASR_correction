...
train_data=pd.read_excel("../hindi_train.xlsx")
test_data=pd.read_excel("../hindi_test.xlsx")
val_data=pd.read_excel("../hindi_val.xlsx")
...
class AttentionLSTM:
    ...
    def build_model(self):
        ...
        # LSTM layers for OCR and ASR outputs
        lstm_ocr, hidden_ocr, _ = LSTM(self.lstm_units, return_sequences=True, return_state=True, use_bias=True,
                                       recurrent_dropout=self.rec_dropout, name='lstm_ocr')(input_ocr_output)
        lstm_asr, hidden_asr, _ = LSTM(self.lstm_units, return_sequences=True, return_state=True, use_bias=True,
                                       recurrent_dropout=self.rec_dropout, name='lstm_asr')(input_asr_output)

        # Attention mechanism applied to the LSTM outputs
        attention_layer = Attention(self.dense_units)
        context_vector_ocr, _ = attention_layer(lstm_ocr, hidden_ocr)
        context_vector_asr, _ = attention_layer(lstm_asr, hidden_asr)
