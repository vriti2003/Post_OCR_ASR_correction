...
from tensorflow.keras.layers import Input, LSTM, Dense, Dropout, concatenate, Layer, Reshape,Bidirectional
...
class AttentionLSTM:
    def __init__(self, time_steps=20, max_ord_length=4, len_per_ord=10, lstm_units=1024, dense_units=40, dropout=0.3, rec_dropout=0.3):
    ...
    def build_model(self):
        ...
        # BiLSTM layers for OCR and ASR outputs
        bilstm_ocr, forward_h_ocr, forward_c_ocr, backward_h_ocr, backward_c_ocr = Bidirectional(
            LSTM(self.lstm_units, return_sequences=True, return_state=True, use_bias=True,
                 recurrent_dropout=self.rec_dropout, name='bilstm_ocr'), merge_mode='concat'
        )(input_ocr_output)
        
        bilstm_asr, forward_h_asr, forward_c_asr, backward_h_asr, backward_c_asr = Bidirectional(
            LSTM(self.lstm_units, return_sequences=True, return_state=True, use_bias=True,
                 recurrent_dropout=self.rec_dropout, name='bilstm_asr'), merge_mode='concat'
        )(input_asr_output)

        # Combine forward and backward hidden states for attention
        hidden_ocr = concatenate([forward_h_ocr, backward_h_ocr], axis=-1)
        hidden_asr = concatenate([forward_h_asr, backward_h_asr], axis=-1)

        # Attention mechanism applied to the BiLSTM outputs
        attention_layer = Attention(self.dense_units)
        context_vector_ocr, _ = attention_layer(bilstm_ocr, hidden_ocr)
        context_vector_asr, _ = attention_layer(bilstm_asr, hidden_asr)
...
attention_lstm = AttentionLSTM(time_steps=TIME_STEPS, max_ord_length=MAX_LEN_ORD, len_per_ord = LENGTH_PER_ORD, lstm_units=1024, dense_units=64, dropout=0.5, rec_dropout=0.5)
model= attention_lstm.build_model()
